    ...
    <div class="col-12 q-mb-sm">
      ...
      >
        <div class="row items-center">
          <div class="col-auto">
            <q-badge :color="__dbg.badgeColor" text-color="white">{{
              __dbg.state
            }}</q-badge>
          </div>
          <div class="col">
            <div class="text-caption">
              Último PATCH: {{ __dbg.lastPatchAt || "—" }} · Resp:
              {{ __dbg.lastStatus || "—" }} · Err: {{ __dbg.lastError || "—" }}
            </div>
          </div>
          <div class="col-auto">
            <q-toggle v-model="__dbg.enabled" label="DBG" dense />
          </div>
        </div>
      </q-banner>
    </div>
    ...
    <div v-else class="row q-col-gutter-md">
      ...
      <div class="col-12 col-md-8">
        ...
        >
          ...
          <q-chip
            v-if="
              progreso.completado || progreso.porcentaje >= COMPLETE_THRESHOLD
            "
            color="positive"
            text-color="white"
            icon="check_circle"
          ...
          >
            <source
              ...
              :type="contenido?.mime_type || 'video/mp4'"
            />
          </video>
        </div>
...
<script setup>
// ✅ Importa Plyr normal (no ESM deep path)
...
// Opciones base
const basePlyrOptions = Object.freeze({
  seekTime: 10,
  controls: [
    "play-large",
    "play",
...
// 🔑 remount cuando cambia src/id
const videoKey = computed(() => {
  const id = contenido.value?.idcontenido || "x";
  const src = contenido.value?.archivo || "y";
  return `${id}::${src}`;
});
...
function showOverlay(refVar, text) {
  ...
  overlayTimer = setTimeout(() => (refVar.value = ""), 700);
}
...
/* ===== Avance ===== */
async function refreshAvanceCurso() {
  try {
    const { idcurso } = route.params;
    const { data } = await api.get(`/cursos/${idcurso}/mi-avance`);
    if (data?.ok && Number.isFinite(data.porcentaje)) {
      avanceCurso.value = Math.max(
        0,
        Math.min(100, Math.round(data.porcentaje))
      );
      return;
    }
  } catch {}
  try {
    const total = videosFlat.value.length || 0;
    if (!total) {
      avanceCurso.value = null;
      return;
    }
    const completados = videosFlat.value.filter(
      (v) =>
        v.completado === true ||
        (v.idcontenido === contenido.value?.idcontenido &&
          (progreso.value.completado ||
            progreso.value.porcentaje >= COMPLETE_THRESHOLD))
    ).length;
    avanceCurso.value = Math.round((completados / total) * 100);
  } catch {
    avanceCurso.value = null;
  }
}
...
function onVisibilityChange() {
  if (document.visibilityState === "hidden") {
    try {
      guardarProgreso(false, { reason: "hidden" });
    } catch {}
  }
}
function onBeforeUnload() {
  try {
    guardarProgreso(false, { reason: "beforeunload" });
  } catch {}
}
...
async function mountPlyr() {
  ...
  const options = { ...basePlyrOptions };
  plyr.value = new Plyr(rawVideoEl.value, options);
  videoRef.value = plyr.value.media;

  // Verificación visual
  requestAnimationFrame(() => {
    const wrapper = rawVideoEl.value?.closest(".plyr");
    const controls = wrapper?.querySelector(".plyr__controls");
    console.log("¿Plyr montado?:", !!wrapper, "controls:", !!controls);
    // @ts-ignore
    window.__plyr = plyr.value;
  });

  let metaReady = false;
  let ultimoGuardadoLocal = 0;
  let yaForzadoPorUmbral = !!progreso.value?.completado;

  videoRef.value.onloadedmetadata = () => {
    metaReady =
      Number.isFinite(videoRef.value.duration) && videoRef.value.duration > 0;
    if (progreso.value?.ultimo_segundo > 5 && metaReady) {
      try {
  ...
  videoRef.value.ontimeupdate = async () => {
    if (!metaReady) return;
    ...
    if (pct !== progreso.value.porcentaje) progreso.value.porcentaje = pct;

    if (current - ultimoGuardadoLocal >= 15) {
      await guardarProgreso(false, { reason: "tick15s" });
      ultimoGuardadoLocal = current;
    }
    if (!yaForzadoPorUmbral && pct >= COMPLETE_THRESHOLD) {
      yaForzadoPorUmbral = true;
      await guardarProgreso(true, { reason: "threshold60" });
      await refreshAvanceCurso();
    }
  };

  videoRef.value.onpause = async () => {
    if ((videoRef.value.currentTime || 0) >= 1) {
      await guardarProgreso(false, { reason: "pause" });
    }
  };

  videoRef.value.onended = async () => {
    await guardarProgreso(true, { reason: "ended" });
    await refreshAvanceCurso();
  };
...
function onKeydown(e) {
  ...
  switch (e.key) {
    ...
    case "J":
      ...
      break;
    case "ArrowUp":
      v.volume = clamp((v.volume ?? 1) + 0.05, 0, 1);
      showOverlay(speedOverlay, `${Math.round((v.volume ?? 1) * 100)}% 🔊`);
      break;
    case "ArrowDown":
      v.volume = clamp((v.volume ?? 1) - 0.05, 0, 1);
      showOverlay(speedOverlay, `${Math.round((v.volume ?? 1) * 100)}% 🔉`);
...
function __dbgSet(state, extra = {}) {
  __dbg.value.state = state;
  __dbg.value.badgeColor =
    state === "ok"
      ? "positive"
      : state === "sending"
      ? "primary"
      ...
      ? "negative"
      : "grey-7";
  Object.assign(__dbg.value, extra);
}
function __now() {
  return new Date().toLocaleTimeString();
}

async function guardarProgreso(force = false, ctx = {}) {
  if (!plyr.value || !contenido.value) return;

  const media = plyr.value.media ?? plyr.value;
  const current = Math.floor(media.currentTime || 0);
  const duration = Math.floor(media.duration || 0);
  if (!duration || (!force && current < 1)) return;

  const payload = {
    idclase: route.params.idclase,
    idcontenido: contenido.value.idcontenido,
    segundo_actual: Math.min(current, duration),
    duracion: Math.max(1, duration),
  };
  ...
  try {
    ...
    const res = await api.patch("/vistas", payload);
    __dbgSet("ok", { lastStatus: res.status });

    const r = res?.data?.data;
    if (r) {
      if (Number.isFinite(r.ultimo_segundo))
        progreso.value.ultimo_segundo = r.ultimo_segundo;
      if (Number.isFinite(r.porcentaje))
        progreso.value.porcentaje = r.porcentaje;
      if (typeof r.completado === "boolean")
        progreso.value.completado = r.completado;
    } else if (force) {
      progreso.value.completado = true;
    }
  } catch (e) {
    const status = e?.response?.status || "ERR";
    const msg = e?.response?.data?.message || e?.message || "unknown";
    __dbgSet("error", { lastStatus: status, lastError: msg });
    if (__dbg.value.enabled)
      $q.notify({
        type: "negative",
        message: `No se pudo guardar progreso (${status})`,
      });
  }
}
...
// Re-monta Plyr cuando cambia el src
watch(
...
/* ✅ Solo oculta controles nativos cuando Plyr ya está montado */
:deep(.plyr) {
  display: block;
  position: relative;
  max-width: 100%;
